...
- 目的を達成するために、気づきを議事録（Smartglasses, web会議）から引っ張ってきてもらい、それを検索プロンプトに自動で入れられるように、かつそのプロンプトを管理サイトなどで可視化できるようにする
    - 目的：「意匠設計で自動化できる箇所見つけたい」
        - 入力1） Web会議で「法規チェックに毎回1〜2時間かかる」と共有。
            - 追加する調査観点：「建築基準法・条例の自動照合」「用途地域別の制約DB化」「図面Lint（規適合の自動指摘）」「是正案の自動提案」を追加。
    -  目的：「figure03, 1x, tesla optimusのような人型ロボットをゼロから作りたいので、figure03, 1x, tesla optimusから出ている情報から必要なハードウェアやソフトウェアのコンポーネントを見つけたい。」
        - 入力1） Google Meetで「Optimusの関節トルク推定が不安定」と報告。 
            - 追加する調査観点：「関節トルク推定方式」「アクチュエータ制御方式」を追加。
        - 入力2） Smart Glass現場メモで「重心制御が難しい」と記録。
            - 追加する調査観点：「二足歩行時の重心制御」「ZMP制御」「足裏センサ」を追加。
